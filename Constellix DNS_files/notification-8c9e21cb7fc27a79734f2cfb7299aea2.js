...
/*
 The Notification variable holds all JavaScript code required for communicating with the server.
...
var Notification = {
    ...
    subscribe: function (options) {
        var defaults = {
                ...
                onMessage: Notification.onMessage
            };
            var notificationRequest = jQuery.extend(defaults, options);
            notificationRequest.onOpen = function (response) {
            console.log('notificationOpen transport: ' + response.transport);
            if (typeof onNotificationOpen == 'function') {
                onNotificationOpen();
            }
        };
        notificationRequest.onReconnect = function (request, response) {
            console.log("notificationReconnect");
        };
        notificationRequest.onError = function (response) {
            console.log('notificationError: ' + response);
        };
        notificationRequest.onTransportFailure = function (errorMsg, request) {
            console.log('notificationTransportFailure: ' + errorMsg);
        };
        notificationRequest.onClose = function (response) {
            console.log('notificationClose: ' + response);
        };
        Notification.socket.subscribe(notificationRequest);
        return notificationRequest;
    },

    unsubscribe: function () {
        Notification.socket.unsubscribe();
    },
...
if (typeof atmosphere == 'undefined') {
    ...
    Notification.socket = jQuery.atmosphere;
} else {
    // if using atmosphere.js
    Notification.socket = atmosphere;
}
var notificationUrl = jQuery("#notificationUrl").val();
jQuery("#notificationUrl").remove();
...
function showNotificationMessage(data) {
    var itemSelector = getMessagesDiv();
    var countSpanSelector = getCountSpanSelector();
    ...
    //if notification is new add it to notification list
    if(!(element.length)) {
        if (countSpanSelector.text() == '' || isNaN(countSpanSelector.text())) {
            count = 1;
            markAllSeenSelector().text(MESSAGES.NOTIFICATIONS.MARK_ALL_SEEN);
        } else {
            count = parseInt(countSpanSelector.text(), 10) + 1;
        }

        countSpanSelector.text(count);
        getHeaderSelector().text(MESSAGES.NOTIFICATIONS.NOTIFICATIONS_COUNT.format([count]));

        itemSelector.prepend('<li> <a id=\"' + id + '\"' + 'cnsdns-data=\'' + data + '\'' + ' onclick=\"onClickNotification(this)\"  class=\"notification-warning active\"><span class=\"time\">0 min(s)</span><i class=\"fa fa-exclamation-triangle\"></i><span class=\"msg\">' + text + '.</span> </a> </li>');
    }
    else { //else replace the properties in the existing one
        element.attr("cnsdns-data", data);
        element.find(".msg").text(text)
    }
}

function getMessagesDiv() {
    return jQuery("li[item='notifications'] ul .scrollthis");
}

function getMessagesDivli() {
    return jQuery("li[item='notifications'] ul .scrollthis li");
}

function getCountSelector() {
    return jQuery("li[item='notifications'] #woo");
}

function getCountSpanSelector() {
    return jQuery("li[item='notifications'] a .badge");
}

function getHeaderSelector() {
    return jQuery("li[item='notifications'] ul .dd-header .unread-noti");
}

function addDefaultNotifcation() {
    addNotification(notificationUrl);
}

function incrementCounter() {
    var countSelector = getCountSelector();
    var countSpanSelector = getCountSpanSelector();
    var count = 0;
    var span;

    if (isNaN(countSelector.text())) {
        count = 1;
        markAllSeenSelector().text(MESSAGES.NOTIFICATIONS.MARK_ALL_SEEN);
    } else {
        count = parseInt(countSpanSelector.text(), 10) + 1;
    }
    countSelector.find(".badge").remove();
    span = '<span class=\"badge\">' + count + '</span>';
    countSelector.append(span);
}
...
function decrementCounter() {
    ...
    var count = parseInt(countSelector.text(), 10) - 1;
...
function incrementTotalUnread() {
    incrementCounter();
    getHeaderSelector().text(MESSAGES.NOTIFICATIONS.NOTIFICATIONS_COUNT.format([getCountSpanSelector().text()]));
}
...
function decrementTotalUnread() {
    ...
    if (getCountSpanSelector().text() != '')
        count = getCountSpanSelector().text();

    getHeaderSelector().text(MESSAGES.NOTIFICATIONS.NOTIFICATIONS_COUNT.format([count]));
}

function onClickNotification(notificationObj) {
    evaluateProperties(notificationObj);
}

function addNotification(url, onMessage) {
    if (url) {
        var completeUrl = contextPath == "/" ? url : contextPath + url;
        var request = {
            type: '',
            // note that the DefaultMeteorHandler uses the header below for setting and getting the broadcaster
            headers: {'X-AtmosphereMeteor-Mapping': url},
            url: completeUrl,
            onMessage: onMessage
        };
        //subscribe notification with the atmosphere
        Notification.subscribe(request);
    }
}

function removeNotification(url){
    if(url){
        Notification.socket.unsubscribeUrl(url);
    }
}
function markAllSeenSelector() {
    return jQuery("#markSeen");
}

function askMarkAllSeen() {
    Dialog.confirm(MESSAGES.NOTIFICATIONS.CONFIRM_MARK_ALL,function (result) {
        if (result) {
            jQuery(getMessagesDivli()).each(function (i) {
                var a = jQuery(this).find('a').get(0);
                markSeen(a);
            });
        }
    });
...
function getPutUrl(id) {
    return jQuery('#notif-url').val() + "/" + id;
}

function markSeen(notificationObj,callback) {
    var id = notificationObj.id;
    var obj = {};
    obj.status = 2; // change status to READ
    if (jQuery("#"+id).hasClass('active')) {
        if (id) { // notification can be without Id
            var url = getPutUrl(id);
            ajaxRequestOld({ url: url,
                type: REQUEST.METHODS.PUT,
                contentType: REQUEST.CONTENT_TYPE.JSON,
                dataType: REQUEST.DATA_TYPE.JSON,
                data: JSON.stringify(obj)}).done(function (response) {
                onSuccessCall(response, id)
                if(callback) {
                    callback();
                }
            });
        ...
        onSuccessCall(undefined, id)
    }

}

function prepareProperties(notificationProperties) {
    if(notificationProperties) {
        var parentType;
        var parentIds;
        var action_type = notificationProperties.action_type;
        var properties;

        if(action_type != null) {
            switch(action_type) {
                case NOTIFICATION_PROPERTIES.ACTION_TYPE.REVIEW:
                    if(notificationProperties.hasOwnProperty(PARENT_NAME.DOMAIN)) {
                        parentType = PARENT_NAME.DOMAIN;
                        parentIds = notificationProperties.domain;
                    }
                    else if(notificationProperties.hasOwnProperty(PARENT_NAME.TEMPLATE)) {
                        parentType = PARENT_NAME.TEMPLATE;
                        parentIds = notificationProperties.template;
                    }
                    else if(notificationProperties.hasOwnProperty(PARENT_NAME.POOL_A)) {
                        parentType = PARENT_NAME.POOL_A;
                        parentIds = notificationProperties.poolA;
                    }
                    else if(notificationProperties.hasOwnProperty(PARENT_NAME.POOL_AAAA)) {
                        parentType = PARENT_NAME.POOL_AAAA;
                        parentIds = notificationProperties.poolAaaa;
                    }
                    else if(notificationProperties.hasOwnProperty(PARENT_NAME.POOL_CNAME)) {
                        parentType = PARENT_NAME.POOL_CNAME;
                        parentIds = notificationProperties.poolCname;
                    }
                    properties = {parentType: parentType, parentIds: parentIds, action: action_type};
                    return properties;
                    break;
            }
        }
...
function evaluateProperties(notificationObj){
    var id = notificationObj.id;
    var notificationPropertiesData = JSON.parse(notificationObj.getAttribute("cnsdns-data"));
    var notificationProperties = prepareProperties(notificationPropertiesData.notificationProperties);
    var action;
    if(notificationProperties)
    {
        action = notificationProperties.action;
    }
    var text = notificationPropertiesData.message;

    switch(action)
    {
        case NOTIFICATION_PROPERTIES.ACTION_TYPE.REVIEW:
            reviewNotification(notificationObj, notificationProperties, text);
            break;
        default:
            defaultNotification(notificationObj, text);
            break;
    }
}

function onSuccessCall(response, notificationId, message) {
    if (!jQuery("#"+notificationId).hasClass('read')) {
        decrementTotalUnread();
    }
    if (message)
        Dialog.info(message, null, "Notification");
    jQuery("[id=" + notificationId+"]").each(function(i) {
        jQuery(this).removeClass('active').addClass('read') ;
    });
}

function createReviewLink(action, parentType, parentIds) {
    if( action != null && parentType != null && parentIds != null  ) {
        var parentTypeAndIds = "type=" +parentType + parentIds.replace(/([0-9]+)(?:,|$)/g, "&ids=$1");
        var action = action.toLowerCase();
        var link = '/uncommittedchanges/'+ action + '?' + parentTypeAndIds;
        return link;
    }
}

function defaultNotification(notificationObj, message)
{
    var obj = {};
    obj.status = 2; // change status to READ
    var id = notificationObj.id;
    ...
    if (jQuery(notificationObj).hasClass('active')) {
        if (!isNaN(id)) {
            clearMessages();
            ajaxRequestOld({ url: url,
                type: REQUEST.METHODS.PUT,
                ...
                dataType: REQUEST.DATA_TYPE.JSON,
                data: JSON.stringify(obj)}).done(function (response) {
                onSuccessCall(response, id, message)
            });
        } else {  // if notification with no id
            onSuccessCall(undefined, id, message)
        }
    } else {
        Dialog.info(message, null, "Notification");
    }
...
function reviewNotification(notificationObj, notificationProperties, text)
{
    markSeen(notificationObj,function(){
        var link = createReviewLink(notificationProperties.action, notificationProperties.parentType, notificationProperties.parentIds);
        window.location.href = link;
    });
}

function callOnClickNotification(element) {
    var notificationObj = jQuery("a#" + element.id);
    if (notificationObj.length) {
        var notificationPropertiesData = JSON.parse(notificationObj.attr("cnsdns-data"));
        if (notificationPropertiesData.type > 0 && notificationPropertiesData.type < 7)
            notificationObj.click();
    }
}
//= require /atmosphere/jquery.atmosphere.js
//= require /js/common/notification/notification.js
//= require /js/common/notification/notificationAction.js
